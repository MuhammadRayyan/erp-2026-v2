...
## ADR-019 — Tenant access history is transactional, append-only, and secret-free

**Status:** Accepted

Every invitation, tenant membership, business-access, or administration-driven session transition must write a tenant-scoped access event inside the same PostgreSQL transaction as the state change. Events preserve actor, target user/email, optional business and invitation correlation, safe before/after metadata, and occurrence time. PostgreSQL rejects event updates and deletes, and the migration-integrity manifest protects the immutability trigger, function, content checks, and composite business scope. Owner administration reads the history only after enforcing active tenant ownership and the `users.manage` entitlement. No-op requests create no events. Metadata must never contain passwords, secrets, invitation/reset tokens, token digests, URLs, email bodies, or session material. Invitation acceptance is a self-service transition, so the accepting user may be both actor and target. Corrections are represented by later events rather than rewriting history.

## ADR-020 — Account structure and lifecycle precede all posting

**Status:** Accepted

Each business owns a tenant-scoped chart of `LedgerAccount` records with explicit class, constrained type, normal balance, contra behavior, header/posting/control kind, manual-posting policy, hierarchy, stable system key, required status, and active/inactive lifecycle. Existing businesses receive a deterministic UAE-oriented small-business template through the forward migration; onboarding installs the same template inside its serializable transaction. System keys identify future posting policies and remain immutable. Required controls cannot be deactivated, system-managed classification and hierarchy cannot be changed, and there is no hard-delete path. PostgreSQL enforces class/type/balance/kind rules, composite hierarchy scope, same-class active header parents, cycle prevention, and active-child lifecycle restrictions. Account create/update/status changes require accounting permissions and entitlement and produce business audit events. This slice stores structure only: it must not create balances, opening entries, journals, tax postings, allocations, or reports. Accounting periods and locks must be implemented next, followed by one balanced, idempotent posting kernel with reversals before any transaction entry is exposed.

## ADR-021 — Accounting periods precede the posting kernel

**Status:** Accepted

Each business owns non-overlapping accounting periods contained within one configured fiscal year. Periods begin open and may move through soft-locked or closed states only through explicit, reasoned, audited transitions. Open periods permit the future journal kernel to accept accounting dates; soft-locked and closed periods reject ordinary posting. Dates become immutable after a period leaves open status, and periods cannot be deleted. PostgreSQL serializes period changes per business, enforces overlap, fiscal-year, state, metadata, and lifecycle invariants, and the application exposes one transaction-bound posting-date guard for all future financial modules. The business fiscal-year start becomes immutable through ordinary settings after periods exist. Reopening preserves the earlier close event. No journal or document posting may be exposed until the balanced posting kernel calls the period guard inside the same transaction as source idempotency, journal creation, and reversal linkage.

## ADR-022 — Posted journals are balanced, idempotent, immutable, and reversible

**Status:** Accepted

All financial modules must use one internal posting service and one PostgreSQL-protected journal model. A journal is created as pending only within a Serializable transaction, receives at least two exact decimal lines, passes tenant/business, base-currency, open-period, active-account, account-kind, manual/control policy, and exact debit-equals-credit validation, then becomes posted before commit. A deferred database trigger forbids committed pending journals. Posted headers and lines cannot be updated or deleted. Source type plus source ID prevents duplicate economic effects, while a payload hash binds each idempotency key to one normalized posting. Deterministic advisory locks serialize source and idempotency identities; recognized Serializable conflicts are retried with bounded backoff and fresh transactions so equivalent concurrent requests converge on the same entry. Corrections use new linked entries. A reversal is one exact opposite posted journal linked to one original; the original remains unchanged, duplicate conflicting reversals and reversal chains are rejected, and the reversal date must pass current period rules. Posting and reversal create audit evidence atomically. The first kernel supports base currency only and exposes no browser or HTTP posting route. Opening balances and every future sales, purchase, bank, inventory, payroll, project, VAT, allocation, and closing workflow must call this kernel rather than write journal tables directly.
