...
Invitation and password-reset emails are written to PostgreSQL before request completion. Invitation creation and its delivery record share one transaction. Workers claim bounded batches with `FOR UPDATE SKIP LOCKED`, recover stale locks, apply bounded exponential retries, respect expiry, and record provider identifiers or terminal errors. Idempotency keys prevent duplicate queue records. Message bodies containing invitation or reset links are scrubbed after sent, failed, expired, or cancelled outcomes while delivery metadata remains available for operations. The internal processing endpoint requires a separate high-entropy worker secret and is not exposed through normal application navigation. SMTP delivery is at-least-once: a process crash after the SMTP provider accepts a message but before PostgreSQL records success can produce a duplicate, so email templates and downstream behavior must remain safe under duplicate delivery. Redis is not introduced until measured throughput or scheduling requirements justify it.
