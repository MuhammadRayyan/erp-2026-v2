...
export async function updateTenantMemberAccess(input: {
  ...
  if (membership.isOwner && input.status === "DISABLED") {
    throw new Error("TENANT_OWNER_PROTECTED");
  }

  const grants = input.businessGrants ?? [];
  for (const grant of grants) {
    const role = getBusinessRole(grant.roleKey);
    if (!role || role.key === "business.owner") {
  ...
  if (businessIds.length > 0) {
    const businessCount = await db.business.count({
      where: { tenantId: input.tenantId, id: { in: businessIds } },
    });
    if (businessCount !== new Set(businessIds).size) {
    ...
    for (const grant of grants) {
      await transaction.businessMembership.upsert({
        ...
        update: {
          ...
          status: grant.status as MembershipStatus,
        },
        create: {
          tenantId: input.tenantId,
          businessId: grant.businessId,
...
export async function revokeTenantInvitation(input: {
  ...
  const result = await db.tenantInvitation.updateMany({
    where: {
