...
export function validatePrivateFile(input: { name: string; contentType: string; bytes: Uint8Array; maxBytes: number }) {
  ...
  if (!(extension in allowed)) throw new Error("FILE_EXTENSION_NOT_ALLOWED");
  const typedExtension = extension as AllowedExtension;
  const acceptedTypes = allowed[typedExtension] as readonly string[];
  ...
  if (!acceptedTypes.includes(input.contentType)) throw new Error("FILE_CONTENT_TYPE_NOT_ALLOWED");
  if (!signatureMatches(typedExtension, input.bytes)) throw new Error("FILE_SIGNATURE_MISMATCH");
  const baseName = path.basename(input.name, typedExtension).replace(/[^a-zA-Z0-9._ -]+/g, "-").trim().slice(0, 100) || "file";
  const safeName = `${baseName}${typedExtension}`;
  return {
    extension: typedExtension.slice(1),
    safeName,
