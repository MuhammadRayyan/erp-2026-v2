...
export function InvitationForm({
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (!response.ok) {
      setMessage(result.message ?? "The invitation could not be delivered.");
      setPending(false);
      return;
    }

    setMessage("Invitation email sent. The link expires in seven days.");
    setEmail("");
    setPending(false);
  }
  ...
  return (
    <form onSubmit={submit} className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-5 shadow-sm">
      <div>
        ...
        <p className="mt-2 text-sm text-[var(--muted)]">The invitation is emailed to the selected address, is valid for seven days, and grants only the chosen business role.</p>
      </div>
      ...
      <div className="mt-5 flex flex-wrap items-center gap-3">
        <button disabled={!canSubmit || pending} className="rounded-xl bg-[var(--brand)] px-4 py-2.5 font-medium text-white disabled:cursor-not-allowed disabled:opacity-50">{pending ? "Sending…" : "Send invitation"}</button>
